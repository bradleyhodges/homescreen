...
export function createHassClient(
        ...
        try {
            ...
            const ready = () => {
                ...
                syncTimer = setTimeout(() => {
                    ...
                    store.setState({
                        status: "error",
                        error: "Home Assistant connected but entity synchronization timed out. Check permissions and retry.",
                        entities: {},
                    });
                }, 30_000);
                store.setState({
            ...
            const reconnectError = (
                ...
                if (error === ERR_INVALID_AUTH) {
                    ...
                    store.setState({
                        status: "reconnecting",
                        error: "Connection interrupted. Home Assistant is reconnecting automatically.",
                    });
            };
            ...
            const removeListeners = () => {
                ...
                nextConnection.removeEventListener(
                    "disconnected",
                    disconnected,
                );
                nextConnection.removeEventListener(
            ...
            const unsubscribe = subscribeEntityStream(
                ...
                () => {
                    ...
                    store.setState({
                        status: "error",
                        error: "Home Assistant could not synchronize entities. Check permissions and retry.",
                        entities: {},
                    });
                },
            );
        ...
        try {
            ...
            if (callback) {
                ...
                if (
                    ...
                    store.setState({
                        status: "error",
                        error: "Authentication was cancelled, expired, or opened outside this browser session. Connect again.",
                    });
                    return;
                }
                await establish(
                    normalizeInstanceUrl(url, env.url.protocol),
                    env,
                    true,
                );
                return;
            ...
            if (generation === current)
                store.setState({
                    status: "error",
                    error: "Could not restore this browser session. Allow session storage and connect again.",
                });
        }
    }
    async function logout() {
        ...
        try {
            ...
            if (generation === current)
                store.setState({
                    error: "Signed out locally, but token revocation could not be confirmed. Remove the refresh token from your Home Assistant profile if needed.",
                });
        }
    }
    const store = createStore<HassState>(() => ({
        ...
        callService: async (domain, service, data, target) => {
            if (
                ...
                throw new Error(
                    "Connect to Home Assistant before calling a service.",
                );
